...
ssize_t	recv_cred(int, void *, size_t, struct xucred *);
...
#define	CONTROL_LEN	(sizeof(struct cmsghdr) + sizeof(struct xucred))

ssize_t
recv_cred(int fd, void *ptr, size_t nbytes, struct xucred *ucredptr)
{
	struct msghdr	msg;
	struct iovec	iov[1];
	...
	ucredptr->cr_ngroups = 0;	/* indicates no credentials returned */
	if (ucredptr && msg.msg_controllen > 0) {
		struct cmsghdr	*cmptr = (struct cmsghdr *) control;

		if (cmptr->cmsg_len != sizeof(struct cmsghdr) + sizeof(struct xucred))
			err_quit("control length = %d", cmptr->cmsg_len);
		if (cmptr->cmsg_level != SOL_SOCKET)
			err_quit("control level != SOL_SOCKET");
		if (cmptr->cmsg_type != SCM_CREDS)
			err_quit("control type != SCM_CREDS");
		memcpy(ucredptr, CMSG_DATA(cmptr), sizeof(struct xucred));
	}
